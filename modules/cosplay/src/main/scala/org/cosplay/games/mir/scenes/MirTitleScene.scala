...
/**
  *
  */
object MirTitleScene extends MirStarStreakSceneBase("title", "bg2.wav"):
    private val state = stateMgr.state
    private val clickSnd = CPSound(s"$SND_HOME/click.wav")
    private val logoImg = CPImage.loadRexXp(s"$IMG_HOME/${state.logoImg}").trimBg()
