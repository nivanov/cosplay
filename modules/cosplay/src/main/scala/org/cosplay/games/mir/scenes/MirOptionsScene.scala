...
/**
  * 
  */
object MirOptionsScene extends MirStarStreakSceneBase("options", "bg1.wav"):
    ...
    private def mkImage(): CPImage =
        ...
        val colGreen = if state.fg == MirStateManager.FG_GREEN then UNSEL else " "
        val colYellow = if state.fg == MirStateManager.FG_YELLOW then UNSEL else " "
        val colWhite = if state.fg == MirStateManager.FG_WHITE then UNSEL else " "
